define(['jquery'], function ($) {	
	...
	// popups init
	function initPopups() {
		lib.each(lib.queryElementsBySelector('.popup-holder'), function(){
			new ContentPopup({
				holder: this
			});
		});
	...
	ContentPopup.prototype = {
		...
		outsideClickHandler: function(e) {
			...
			var matchNodes = function(target, list) {
				var result = false;
				lib.each(list, function(index, tmpNode) {
					if(target === tmpNode) {
						result = true;
					}
				});
			...
			while(currentNode.parentNode) {
				foundFlag = matchNodes(currentNode, this.popup);
				foundFlag |= matchNodes(currentNode, this.btnOpen);
				if(foundFlag) {
					return;
				}
